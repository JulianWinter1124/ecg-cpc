            ...
            with h5py.File(self.current_file, 'r') as f:
                ...
                if not data is None:
                    while (index+1)*self.window_size*self.n_windows+offset <= len(data):
                        yield np.swapaxes(data[index*self.window_size*self.n_windows+offset : (index+1)*self.window_size*self.n_windows+offset, 0:2].reshape((self.n_windows, self.window_size, -1)), 1, 2)
            ...
            with h5py.File(self.current_file, 'r') as f:
                ...
                while (index + 1) * self.window_size * self.n_windows + offset <= len(data):
                    yield np.swapaxes(data[index * self.window_size * self.n_windows + offset: (
                                                                                                           index + 1) * self.window_size * self.n_windows + offset, 0:2].reshape(
                        (self.n_windows, self.window_size, -1)), 1, 2) #TODO: MAKE chosen channel index selectable for each dataset
                    index += self.n_windows
            file_index += 1
