            ...
            for root, dirs, files in os.walk(f, followlinks=True):
                ...
                if len(dirs) == 0 and len(files) > 0:  # leaf dir (model?)
                    ...
                    if 'model_variables.txt' in files:
                        ...
                        model_descriptions[name]['MaxPool1d'] = count_key_in_dict(data,
                                                                                  'torch.nn.modules.pooling.MaxPool1d')
                        model_descriptions[name]['AdaptiveAvgPool1d'] = count_key_in_dict(data,
                        ...
                        model_descriptions[name]['BatchNorm1d'] = count_key_in_dict(data,
            ...
            if len(dirs) == 0 and len(files) > 0:  # leaf dir (model?)
                ...
                if 'model_variables.txt' in files:
                    ...
                    if "sampling_mode" in content:
                        model_descriptions[name]['sampling mode'] = content.split('"sampling_mode": ')[1].split(',')[0][
                                                                    1:-1]
                    else:
                        model_descriptions[name]['sampling mode'] = 'none'
                    if '"downstream_model":' in content:
                        model_descriptions[name]['downstream model'] = \
