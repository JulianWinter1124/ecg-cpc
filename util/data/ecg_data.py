...
class ECGData():
    def __init__(self, base_directory='/home/julian/Datasets/ptb-diagnostic-ecg-database-1.0.0/'):
        ...
        self.default_key_function_dict['reason for admission'] = lambda x: self.filter_comment('reason for admission',
                                                                                               x), 10
    ...
    def partition_data(self, data, window_size=3000, overlap=0.5, store_in_array=True, align_right=True,
        ...
        if verbose:
            print('The window of size %d will be shifted by %f. The total data used is %d' % (
                window_size, shift, samples))
    ...
    def convert_dat_to_h5_partitioned(self, storage_path, dat_file_paths, window_size=3000, overlap=0.5,
                                      align_right=True, overwrite=True, verbose=True):
    ...
    def convert_dat_to_h5(self, storage_path, dat_file_paths, channels=None, normalize_data=False, pca_components=0,
                          use_labels=False, use_header=False, terms=None, key_function_dict=None, verbose=True):
        if not os.path.exists(storage_path):
            os.makedirs(storage_path)
        for f in dat_file_paths:
    ...
    def convert_dat_to_h5_context_task(self, storage_path, dat_file_paths, window_size=3000, overlap=0.5, time_in=5,
                                       time_out=3, align_right=True, verbose=True):
        if not os.path.exists(storage_path):
            os.makedirs(storage_path)
        for f in dat_file_paths:
            data = self.read_signal(os.path.join(self.BASE_DIR, f))
