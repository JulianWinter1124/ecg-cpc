...
def create_few_labels_splits(train_fraction, val_fraction, test_fraction):
    ...
    ptbxl_challenge.random_train_split_with_class_count(train_fraction, val_fraction, test_fraction,
                                                        filename_overwrite=filename)
    cpsc_challenge.random_train_split_with_class_count(train_fraction, val_fraction, test_fraction,
                                                       filename_overwrite=filename)
    cpsc2_challenge.random_train_split_with_class_count(train_fraction, val_fraction, test_fraction,
                                                        filename_overwrite=filename)
    georgia_challenge.random_train_split_with_class_count(train_fraction, val_fraction, test_fraction,
                                                          filename_overwrite=filename)
    ...
    print(ecg_datasets2.count_merged_classes(
        [nature, ptbxl_train, ptbxl_val, t1, georgia_train, georgia_val, t2, cpsc_train, cpsc_val, t3, cpsc2_train,
         cpsc2_val, t4]))
...
def create_few_labels_splits_like_old(train_fraction):
    ...
    ptbxl_new_train_split, _, _ = ptbxl_train.random_train_split_with_class_count(new_train_fraction, 0.0, 0.0,
                                                                                  save=False)
    georgia_new_train_split, _, _ = georgia_train.random_train_split_with_class_count(new_train_fraction, 0.0, 0.0,
                                                                                      save=False)
    cpsc_new_train_split, _, _ = cpsc_train.random_train_split_with_class_count(new_train_fraction, 0.0, 0.0,
                                                                                save=False)
    cpsc2_new_train_split, _, _ = cpsc2_train.random_train_split_with_class_count(new_train_fraction, 0.0, 0.0,
                                                                                  save=False)

    ecg_datasets2.save_train_test_split(os.path.join(ptbxl_train.BASE_DIR, filename), ptbxl_new_train_split,
                                        ptbxl_val.files, t1.files)
    ecg_datasets2.save_train_test_split(os.path.join(georgia_train.BASE_DIR, filename), georgia_new_train_split,
                                        georgia_val.files, t2.files)
    ecg_datasets2.save_train_test_split(os.path.join(cpsc_train.BASE_DIR, filename), cpsc_new_train_split,
                                        cpsc_val.files, t3.files)
    ecg_datasets2.save_train_test_split(os.path.join(cpsc2_train.BASE_DIR, filename), cpsc2_new_train_split,
                                        cpsc2_val.files, t4.files)
...
def create_few_labels_splits_minimum_cut(min_cut=20):
    ...
    class_counts, counted_classes = ecg_datasets2.count_merged_classes(
        [ptbxl_train, georgia_train, cpsc_train, cpsc2_train])
    ...
    ptbxl_new_train_split, _, _ = ptbxl_train.random_train_split_with_class_count_mins(buckets[0],
                                                                                       np.zeros_like(buckets[0]),
                                                                                       np.zeros_like(buckets[0]),
                                                                                       save=False)
    georgia_new_train_split, _, _ = georgia_train.random_train_split_with_class_count_mins(buckets[1],
                                                                                           np.zeros_like(buckets[1]),
                                                                                           np.zeros_like(buckets[1]),
                                                                                           save=False)
    cpsc_new_train_split, _, _ = cpsc_train.random_train_split_with_class_count_mins(buckets[2],
                                                                                     np.zeros_like(buckets[2]),
                                                                                     np.zeros_like(buckets[2]),
                                                                                     save=False)
    cpsc2_new_train_split, _, _ = cpsc2_train.random_train_split_with_class_count_mins(buckets[3],
                                                                                       np.zeros_like(buckets[3]),
                                                                                       np.zeros_like(buckets[3]),
                                                                                       save=False)
