...
class BaselineNet(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, out_classes, verbose):
        ...
        self.tcn_block = nn.Sequential(
            weight_norm(nn.Conv1d(in_channels=in_channels, out_channels=in_channels, kernel_size=3, stride=2, padding=0,
                                  dilation=1)),
            nn.ReLU(),
            nn.Dropout(0.2),

            weight_norm(nn.Conv1d(in_channels=in_channels, out_channels=in_channels, kernel_size=3, stride=1, padding=0,
                                  dilation=2)),
            nn.ReLU(),
            nn.Dropout(0.2),
        )
    ...
    def forward(self, x, y=None):
        ...
        x = self.fc(
            x)  # Like in https://github.com/locuslab/TCN/blob/master/TCN/mnist_pixel/model.py TODO: why is that okay?
