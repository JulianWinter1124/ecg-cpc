...
class BaselineNet(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, out_classes, verbose):
        ...
        self.convs = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Conv1d(in_channels=32, out_channels=32, kernel_size=3, dilation=2, stride=1),
            nn.ReLU(),
            nn.Conv1d(in_channels=32, out_channels=64, kernel_size=3, dilation=4, stride=1),
            nn.ReLU()

        )
        self.downsample = nn.Conv1d(in_channels=110, out_channels=1, kernel_size=1)
