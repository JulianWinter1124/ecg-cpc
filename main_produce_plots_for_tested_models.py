    ...
    try:
        ...
        with open(os.path.join(model_folder, 'train_params.txt'), 'r') as file:
                ...
                if 'norm_fn=' in content:
                    ...
                    if int(timestamp) > 1637682016:
                        attrs["Train Normalization Function"] = 'normalize_std_scaling'
                    else:
                        attrs["Train Normalization Function"] = 'normalize_minmax_scaling_different'
        ...
        with open(os.path.join(model_folder, 'params.txt'), 'r') as file:
            ...
            if 'norm_fn=' in content:
                ...
                if int(timestamp) > 1638140400: #oh god
        ...
        if is_cpc:
            ...
            try:
                ...
                if not attrs['strided']:
                    ...
                    attrs['Encoder'] = list(data["architectures_cpc.cpc_combined.CPCCombined"]["_modules"][""]["cpc_model"][cpc_type]["_modules"][""]["encoder"]
                                            ["architectures_cpc.cpc_encoder_as_strided.StridedEncoder"]["_modules"][""]["cpc_encoder"])[0].split('.')[-2]
...
def create_paper_metrics(model_folders, root_path, data_loader_index=0, average_only=False, long_tables=False, cut_name_attrs=False, use_attrs_in_name=False, include_attrs_in_table=False, save_csv=False,
    ...
    if save_csv:
        auc_dff.to_csv(root_path, 'All_attributes_with_scores2.csv')
    try:
...
def create_latex_table_from_csv(csv_file, out_path=None, out_name=None, auto_strip_cols=True, average_only=False, long_tables=False, use_attrs_in_name=False, save_to_all_dirs=True):
    ...
    try:
        dff.dataframe.drop(columns=['Model Path', 'train timestamp'], inplace=True)
    except:
        print(['Model Path', 'train timestamp'], 'not found in df')

    dff.to_latex(out_path, out_name+'.tex', caption='AUC score',
                     label='tbl:auc-' + out_name, long_tables=long_tables, only_tabular_environment=True)
...
if __name__ == '__main__':
    ...
    paths = [
        ...
        # '/home/julian/Downloads/Github/contrastive-predictive-coding/models/03_02_22-14-40-test|(2x)cpc'
        # '/home/julian/Downloads/Github/contrastive-predictive-coding/models/11_02_22-15-10-test|(8x)cpc'
        # '/home/julian/Downloads/Github/contrastive-predictive-coding/models/25_06_21-16-test|bl_TCN_down/24_06_21-16-train|(2x)bl_TCN_block+(2x)bl_TCN_down+(2x)bl_TCN_flatten+(2x)bl_TCN_last+(2x)bl_cnn_v15+(2x)bl_rnn_simplest_lstm+bl_MLP+bl_alex_v2+bl_cnn_v7',
        ...
        #'/home/julian/Downloads/Github/contrastive-predictive-coding/models/15_02_22-15-09-test|(4x)cpc',
        '/home/julian/Downloads/Github/contrastive-predictive-coding/models/15_02_22-17-57-test|bl_cnn_v14+bl_cnn_v2+bl_cnn_v8'
