...
class CPC(nn.Module):
    def __init__(self, encoder_model, autoregressive_model, predictor_model, timesteps_in, timesteps_out,
                 timesteps_ignore=0):
    ...
    def forward(self, x_windows, n_timesteps_in, n_timesteps_out, hidden, verbose=False):
        ...
        if n_windows != n_timesteps_in + n_timesteps_out:
        ...
        for k in range(0, n_timesteps_out):  # Do this for each timestep
            latent_k = self.encoder(x_windows[-n_timesteps_out + k])
            pred_k = self.predictor(context, k)  # Shape (Batches, latents)
            inner_f = torch.mm(latent_k, pred_k.transpose(0, 1))
            correct += torch.sum(torch.eq(torch.argmax(self.softmax(inner_f)), torch.arange(0,
                                                                                            n_batches).cuda()))  # how many correct? Also I don't understand this completely
            log_softmax_k = self.lsoftmax(torch.diag(inner_f))
            loss += torch.sum(log_softmax_k)
        loss /= (
                    n_timesteps_out * n_batches * -1.0)  # Unterschiedlich gewichten auch moeglich. Oder mehr als nur n_batches-1 negative samples
        # loss /= (n_timesteps_out * 1.0) #Man könnte auch jeden Timestep unterschiedlich gewichten. Nahe = wichtiger als entfernt
        accuracy = correct / (n_batches * n_timesteps_out)
...
class AutoRegressor(nn.Module):
    ...
    def init_hidden(self, batch_size, use_gpu=True):
        if use_gpu:
            return torch.zeros(1, batch_size, self.hidden_size).double().cuda()
        else:
            return torch.zeros(1, batch_size, self.hidden_size)

    def forward(self, x, hidden):
        # Input is (seq, batch, latents) maybe (1, .., ...)
