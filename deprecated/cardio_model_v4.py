...
class CPC(nn.Module):
    ...
    def forward(self, x_windows, n_timesteps_in, n_timesteps_out, hidden, verbose=False):
        ...
        for k in range(0, n_timesteps_out): #Do this for each timestep
            ...
            correct += torch.sum(torch.eq(torch.argmax(self.softmax(inner_f)), torch.arange(0, n_batches).cuda())) #how many correct? Also I don't understand this completely
...
class AutoRegressor(nn.Module):
    ...
    def init_hidden(self, batch_size, use_gpu=True):
        if use_gpu: return torch.zeros(1, batch_size, self.hidden_size).double().cuda()
