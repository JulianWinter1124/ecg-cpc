...
class DownstreamLinearNet(nn.Module):
    ...
    def __init__(self, cpc_model, latent_size, context_size, out_classes, use_context=True, use_latents=False, freeze=True, verbose=False):
        super().__init__()
        ...
        if self.use_latents:
            ...
            self.linear_latents = nn.Linear(57*latent_size, out_classes)
        self.activation = nn.Sigmoid()
        #self.loss = nn.KLDivLoss()
        self.loss = nn.MSELoss()


    def forward(self, X, cpc_hidden=None, y=None):
        latents, context, cpc_hidden = self.cpc_model(X, hidden=cpc_hidden) #e.g shape
        if self.verbose: print('latents', latents.shape, 'context', context.shape)
    ...
    def init_hidden(self, batch_size, use_gpu):
        return self.cpc_model.init_hidden(batch_size, use_gpu)
