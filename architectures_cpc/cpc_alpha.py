...
class CPC(nn.Module):
    ...
    def __init__(self, encoder, autoregressive, predictor, timesteps_in, timesteps_out, latent_size, alpha=1.0,
    ...
    def forward(self, X, y=None, hidden=None):
        ...
        encoded_x = self.encoder(
            X)  # encode whole data: shape: (batch, latent_size, length->out_length) #THIS IS NOT HOW ITS SHOWN IN THE PAPER
        ...
        if self.sampling_mode == 'same':
            for k in range(self.timesteps_out):
                ...
                for step in range(pred_latent.shape[0]):  # TODO: can this be broadcasted?
                    softmax = self.lsoftmax(
                        torch.mm(encoded_latent[step], pred_latent[step].T))  # output: (Batches, Batches)
        ...
        if self.sampling_mode == 'all':
            ...
            for k in range(self.timesteps_ignore, self.timesteps_out):
                ...
                if self.normalize_latents:
                    pred_latent = batch * pred_latent / torch.norm(pred_latent, p=2, dim=-1,
                                                                   keepdim=True)  # Multiplying with batch so softmax stable
