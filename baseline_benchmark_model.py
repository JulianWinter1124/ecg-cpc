...
class BaselineNet(nn.Module):
    ...
    def forward(self, X, y=None):
        ...
        if not y is None:
            ...
            accuracies.append(torch.sum(logits[mask] >= 0.1)-torch.sum(logits[inverse_mask] >= 0.1))/torch.sum(mask) #Counts the classes that have been correctly predicted - wrongly predicted with certain prob
